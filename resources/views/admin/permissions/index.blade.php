    ...
    <div class="flex flex-col h-full overflow-hidden p-2 sm:p-4 md:p-8">
        <div class="mb-6 flex-none flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <form method="POST" action="{{ route('admin.permissions.reset') }}"
                ...
                @csrf
                @method('DELETE')
                <button type="submit" class="px-3 py-2 text-sm font-semibold bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors focus:ring-2 focus:ring-red-500 focus:outline-none">
                    Reset
                </button>
            </form>
        </div>
        ...
        @if (session('success'))
            <div class="mb-4 flex-none p-3 bg-green-50 border border-green-200 text-green-800 rounded-lg text-sm font-medium">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form method="GET" class="mb-6 flex-none flex gap-2">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-700 font-bold text-white rounded-lg text-sm hover:bg-blue-800 transition-colors focus:ring-2 focus:ring-blue-600 focus:outline-none">
                Cari
            </button>
            @if (request('search'))
                <a href="{{ route('admin.permissions.index') }}" class="px-4 py-2 bg-slate-200 text-slate-800 font-bold rounded-lg text-sm hover:bg-slate-300 transition-colors focus:ring-2 focus:ring-slate-400 focus:outline-none">
                    Reset
                </a>
            @endif
        </form>
                        ...
                        <tr>
                            ...
                            @foreach ($availableRoles as $role)
                                <th class="px-4 py-4 text-center font-bold text-slate-900">
                                    <span class="inline-block px-3 py-1 bg-blue-100 text-blue-900 border border-blue-200 rounded-md text-xs font-bold uppercase tracking-wider">
                                        {{ $role }}
                                    </span>
                                </th>
                            @endforeach
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @foreach ($grouped as $section => $perms)
                            ...
                            <tr class="bg-slate-100 border-t-2 border-slate-300">
                                <td colspan="{{ count($availableRoles) + 1 }}" class="px-6 py-3 font-bold text-slate-800 uppercase tracking-wider text-xs">
                                    {{ $section }}
                                </td>
                            </tr>
                            <!-- Pages Rows -->
                                ...
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    @foreach ($availableRoles as $role)
                                        <td class="px-4 py-3 text-center align-middle">
                                            <label class="inline-flex items-center cursor-pointer m-0 p-2 hover:bg-slate-100 rounded focus-within:ring-2 focus-within:ring-green-500 transition-colors">
                        ...
                        @if(count($grouped) === 0)
                            <tr>
                                <td colspan="{{ count($availableRoles) + 1 }}" class="px-6 py-12 text-center text-slate-500 font-medium">
