<header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 z-10 shrink-0 w-full">
    <div class="flex items-center gap-3">
        <!-- Hamburger Button -->
        <button onclick="toggleSidebar()" class="lg:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-lg focus:outline-none">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
            </svg>
        </button>
        
        <div class="font-semibold text-slate-800 hidden sm:block">
            {{ 'Pemeriksa Panduan Kurikulum' }}
        </div>
        <div class="font-semibold text-slate-800 sm:hidden text-base">
            {{ 'Pemeriksa' }}
        </div>
    </div>
