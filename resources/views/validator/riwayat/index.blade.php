...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-slate-700 mt-1 text-base">Lihat semua riwayat penilaian dan validasi dokumen kurikulum yang pernah Anda lakukan.</p>
    </div>

    <!-- Filter Card -->
    <div class="bg-white rounded-xl shadow-sm border border-slate-300 p-5">
        <form action="{{ route('validator.riwayat.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            
            <!-- Program Studi -->
            <div>
                <label for="prodi_id" class="block text-sm font-semibold text-slate-800 mb-1">Program Studi</label>
                <select name="prodi_id" id="prodi_id" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            ...
            <div>
                ...
                <input type="date" name="date_from" id="date_from" value="{{ $dateFrom }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>

            <!-- Tanggal Sampai -->
            <div>
                <label for="date_to" class="block text-sm font-semibold text-slate-800 mb-1">Sampai Tanggal</label>
                <input type="date" name="date_to" id="date_to" value="{{ $dateTo }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
            </div>

            <!-- Buttons -->
            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors focus:ring-2 focus:ring-blue-600 focus:outline-none shadow-sm">
                    Filter
                </button>
                <a href="{{ route('validator.riwayat.index') }}" class="flex-1 text-center bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-2 px-4 rounded-lg text-sm border border-slate-300 transition-colors focus:ring-2 focus:ring-slate-400 focus:outline-none">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-300 overflow-hidden">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-500">
                                    <svg class="w-12 h-12 mb-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                    </svg>
                                    <p class="text-base font-medium text-slate-700">Tidak ada riwayat validasi ditemukan</p>
                                    <p class="text-sm mt-1">Belum ada riwayat validasi atau tidak ada data yang cocok dengan filter yang dipilih.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if ($auditLogs->hasPages())
            <div class="px-6 py-4 border-t border-slate-300 bg-slate-50">
