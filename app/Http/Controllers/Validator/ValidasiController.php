...
class ValidasiController extends Controller
{
    ...
    public function store(Request $request, $submission_id)
    {
        ...
        $transition = $submission->canTransitionTo($submissionStatus, Auth::user()->role);
        if (!$transition['valid']) {
        ...
        $submission->update(['status' => $submissionStatus, 'updated_at' => now()]);

        // Log the validation action via audit log
        if ($validated['status'] === 'disetujui') {
            AuditLogService::logApproval($submission);
        } elseif ($validated['status'] === 'revisi') {
            AuditLogService::logRevision($submission);
        } elseif ($validated['status'] === 'ditolak') {
            AuditLogService::logRejection($submission);
        }
